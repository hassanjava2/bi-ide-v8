...
class BrainDaemon:
    ...
    def run_cycle(self):
        ...
        # ═══ امتحان كل N دورات ═══
        if self.cycle % EVAL_EVERY_N_CYCLES == 0:
            logger.info(f"\n📝 Self-eval...")
            try:
                evals = self.evaluator.evaluate_all()
                grades = {r["grade"]: 0 for r in evals}
                for r in evals:
        ...
        # ═══ المجلس كل N دورات ═══
        if self.cycle % COUNCIL_EVERY_N_CYCLES == 0:
            logger.info(f"\n🏛️ Council deciding...")
            try:
                decisions = self.council.decide()
                executed = self.council.execute_decisions(decisions)
